...
namespace bt709_to_bt2020
{
    class Program
    {
        ...
        static double[] inv_quantization_ycbcr(ref double[] input_ycbcr)
        {
            ...
            nonlinear_ycbcr[1] = (input_ycbcr[1] / 4 - 128) / 224;
            nonlinear_ycbcr[2] = (input_ycbcr[2] / 4 - 128) / 224;
        ...
        {
            Console.WriteLine("BT 2087 YCbCr Converter (BT709 to BT2020 10 Bit)");
            Console.WriteLine("------------------------------------------------\n");
            double[] input_codes = new double[3];// { 250, 409, 960 };
            string[] color_channels = new string[3] {"Y" , "Cb" , "Cr"};

            for (int i = 0; i < 3; i++)
            {
                Console.WriteLine("Input 10 Bit " + color_channels[i] + " : ");
                input_codes[i] = double.Parse(Console.ReadLine());
            }
            ...
            for (int i = 0; i < 3; i++)
            {
                Console.WriteLine("Output Code " + color_channels[i] );
